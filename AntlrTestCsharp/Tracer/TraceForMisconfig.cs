...
{
    public class TraceForMisconfig
    {
        string code;
        List<AspCommentObj> listComment;
        public List<ItemObject> listItem { get; set; }
        public TraceForMisconfig(string code, string pathFile)
        {
            this.code = code;
            listComment = new List<AspCommentObj>();
            listItem = new List<ItemObject>();
            ProcessTraceMisconfig(code, pathFile);

        }

        private void ProcessTraceMisconfig(string code, string pathFile)
        {
            string pattern = "<%--(.[\\s\\S]*?)--%>";
            ...
            pattern = @"debug.*true";
            Regex debugregex = new Regex(pattern);
            MatchCollection debugList = debugregex.Matches(code);

            if (debugList.Count > 0)
            {
                foreach (Match item in debugList)
                {
                    if (listComment.Count > 0)
                    {
                        ...
                        if (!iscommented)
                        {
                            ...
                            if (value != "csrftoken")
                            {
                                ...
                                ItemObject obj = new ItemObject(700, "Web.config", null, pathFile, line, "WARNING");
                                listItem.Add(obj);
                            }
